...
<head>
  ...
  <title>Home decor - promocoes</title>
  ...
  <div class="container-justify-content-center">
    ...
    <div class="nav-scroller py-3 mb-2">
      <ul class="nav nav-underline flex nav-fill">
        ...
        <li class="nav-item">
          <a class="nav-link" href="#formulario" style="color: black"> Promoções </a>
        </li>
      </ul>
    </div>
  </div>
    ...
    <div class="card " style="padding:20px; margin-top: 20px; align-self:center" id="formulario">
        <form action="{{ route('promocao.search') }}" method="post" class="row g-4">
            ...
            <button class="btn btn-secondary btn-sm">
                <a href="{{ route('promocao.create') }}" style="color:white; text-decoration:none">Cadastrar</a>
            </button>

            <button class="btn btn-secondary btn-sm">
                <a href="{{ route('promocao.chart') }}" 
                   target="_blank" 
...
<br>
      <div class="d-flex flex-row justify-content-center flex-wrap">
        ...
        <div class="card-body">
          <img src="/storage/{{ $nome_imagem }}" width="250px" alt="imagem">
          <h5 class="card-title">{{ $item->produto->nome ?? '' }}</h5>
          <p class="card-text">{{$item->descricao}}</p>
          <p class="card-text">NOVO PREÇO: R${{$item->novo_valor}}</p>
            <button type="button" class="btn btn-secondary">
            <a href="{{route('promocao.edit', $item->id)}}" style="color:white;text-decoration:none">Editar</a>
            </button>
            <button type="button" class="btn btn-secondary">
                <a href="{{route('promocao.destroy', $item->id)}}" style="color:white;text-decoration:none"
