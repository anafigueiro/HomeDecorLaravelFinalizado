...
<body>
  ...
  <div class="container-justify-content-center">
    ...
    <h3 class="h3" style="text-align: center"> Lista de ouvidorias</h3>
    <div class="card" style="padding:20px; margin-top: 20px">
        <form action="{{ route('ouvidoria.search') }}" method="post" class="row g-3">
            @csrf <!-- cria um hash de segurança -->
            <div class="col-md-6">
                <select class="btn btn btn-secondary btn-sm dropdown-toggle" name="tipo" >
                    <option value="contato_nome">Nome do contato</option>
                    <option value="sugestao">Sugestão</option>
                    <option value="dataRegistro">Data de registro</option>
            ...
            <button class="btn btn-secondary btn-sm">
                <a href="{{ route('ouvidoria.create') }}" style="color:white; text-decoration:none">Cadastrar</a>
            </button>
            <button class="btn btn-secondary btn-sm">
              <a href="{{ route('ouvidoria.report') }}" 
                 target="_blank" 
                 style="color:white; text-decoration:none">Relatório</a>
          </button>
        </form>
    </div>
    <table border="1" class="table table-bordered" style="margin-top: 30px">
        ...
        @foreach ($ouvidorias as $item)
            <tr>
                ...
                <td><a href="{{route('ouvidoria.edit', $item->id)}}">Editar</a></td>
                <td><a href="{{route('ouvidoria.destroy', $item->id)}}"
