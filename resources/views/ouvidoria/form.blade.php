...
<body>
 ...
 <div class="container-justify-content-center">
  ...
  <main class="container"> <br>
    ...
    <div id="formulario">
      <h3 class="h3" style="text-align: center"> Ouvidoria</h3>
    </div>
    ...
    @php
       // dd($contato); // é igual ao var_dump()
        if(!empty($ouvidoria->id)){
          $route = route('ouvidoria.update', $ouvidoria->id);
        } else {
          $route = route('ouvidoria.store');
        }
    @endphp
    ...
    <form action="{{ $route }}" method="post" class="row g-3">
        ...
        @if (!empty($ouvidoria->id))
            @method('PUT')
        @endif
        <input type="hidden" name="id" value="@if(!empty($ouvidoria->id)){{$ouvidoria->id}}@elseif (!empty(old('id'))){{old('id')}}@else{{''}}@endif">

         {{-- dd($contatos) --}}
        <div class="col-md-6">
        <label for="">Nome do contato</label><br>
              <select name="contato_id" class="form-select">
        ...
        <div class="col-md-6">
            <label for="">Data de registro</label><br>
            <input type="date" name="dataRegistro" class="form-control"
                value="@if (!empty($ouvidoria->dataRegistro)){{$ouvidoria->dataRegistro}}@elseif(!empty(old('dataRegistro'))) {{old('dataRegistro')}} @else{{''}}@endif">
        </div>
        
        <div class="col-md-12">
            <label for="">Sugestão</label><br>
            <input type="text" name="sugestao" class="form-control"
                value="@if(!empty($ouvidoria->sugestao)) {{$ouvidoria->sugestao}} @elseif (!empty(old('sugestao'))){{old('sugestao')}}@else{{''}}@endif">
        </div>

        <div class="col-md-12">
            <label class="form-label">Observação</label><br>
            <input type="text" name="observacao"  class="form-control" name="ajuda" aria-label="With textarea"
                value="@if (!empty(old('observacao'))){{old('observacao')}}@elseif(!empty($ouvidoria->observacao)){{$ouvidoria->observacao}}@else{{''}}@endif">
        </div>
        ...
        <div class="col-12">
            ...
            <button type="button" class="btn btn-secondary"> 
                <a href="{{ route('ouvidoria.index') }}" style="color: white; text-decoration: none">Ver Registros</a>
            </button>
        </div>
    </form>
    </div>
